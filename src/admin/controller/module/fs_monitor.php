...
namespace Opencart\Admin\Controller\Extension\FSMonitor\Module;
...
class FSMonitor extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link($this->_route, 'user_token=' . $this->session->data['user_token'] . '&page={page}', 'SSL')
		]);
	 ...
	 **/
	public function reloadWidget(): void {
	 ...
	 * @return 	int 	scan_id
	 **/
	private function createScan($name): int {
		$this->load->model($this->_route);
	 ...
	 * @return void
	 **/
	public function generateDefaultSettings(): void {
		$this->load->model($this->_route);
		$this->language->load($this->_route);
	 ...
	 * @return bool
	 **/
	private function validateScan(): bool {
		if (!$this->user->hasPermission('modify', $this->_route)) {
	 ...
	 * @return 	array
	 **/
	private function formatFile(string $file_name, array $file_data): array {
