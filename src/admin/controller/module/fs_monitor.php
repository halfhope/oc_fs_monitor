...
class ControllerModuleFsMonitor extends Controller
{
	...
	public function install() {
		$this->{$this->_model}->checkAndInstall(true);
	}
	...
	public function index()
	{
		...
		$this->{$this->_model}->checkAndInstall(false);
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateScan()) {
			...
			if (!isset($this->request->post['scan_name']) || empty($this->request->post['scan_name'])) {
				...
				$this->redirect($this->url->link($this->_module_route, 'token=' . $this->session->data['token'], 'SSL'), 301);
			...
			$this->url->link($this->_module_route, 'scan_id=' . (int) $scan_id . '&token=' . $this->session->data['token'], 'SSL');
		}
		...
		$this->data = $data;
		$this->template = $this->_module_route . '/main.tpl';
	...
	public function viewScan()
	{
		...
		if (isset($this->request->get['scan_id'])) {
			...
			$this->redirect($this->url->link($this->_module_route, 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$this->data = $data;
		$this->template = $this->_module_route . '/view_scan.tpl';
	...
	public function settings()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateSettings()) {
			...
			$this->redirect($this->url->link($this->_module_route . '/settings', 'token=' . $this->session->data['token'], true));
		...
		$this->data = $data;
		$this->template = $this->_module_route . '/settings.tpl';
	...
	public function rename()
	{
		if ($this->user->hasPermission('modify', $this->_module_route)) {
			if (isset($this->request->post['scan_name']) && !empty($this->request->post['scan_name'])) {
				...
				$this->redirect($this->url->link($this->_module_route . '/viewScan',  'scan_id=' . (int)$this->request->get['scan_id'] . '&token=' . $this->session->data['token'], true));
	...
	public function viewFile()
	{
		...
		if ($this->user->hasPermission('modify', $this->_module_route)) {
			...
			$this->data = $data;
			$this->template = $this->_module_route . '/view_file.tpl';
	...
	public function widget()
	{
		...
		if ($scan) {
			...
			$this->data = $data;
			$this->template = $this->_module_route . '/widget.tpl';
	...
	public function reloadWidget()
	{
		if ($this->validateScan()) {
			...
			$scan_id = $this->createScan($this->language->get('text_dashboard_scan'));
