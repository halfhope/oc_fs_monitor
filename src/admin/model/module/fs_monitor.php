...
namespace Opencart\Admin\Model\Extension\FSMonitor\Module;
class FSMonitor extends \Opencart\System\Engine\Model {

	private function pack_data(array $array): string {
		return base64_encode(gzdeflate(json_encode($array)));
	}

	private function unpack_data(string $data): array {
		return json_decode(gzinflate(base64_decode($data)), true);
	}
	
	private function editSetting(string $key, string $value, string $code, int $store_id = 0): bool {
	...
	public function addScan(string $name, array $files, int $scan_size, string $user = NULL): int {

		$user_name = (!empty($user)) ? $user : $this->user->getUserName();
		...
		$last_scan = $this->db->query("SELECT * FROM `" . DB_PREFIX . "security_filesystem_monitor_data` AS sfmd LEFT JOIN `" . DB_PREFIX . "security_filesystem_monitor_generated` AS sfmg ON sfmg.`scan_id` = sfmd.`scan_id` WHERE sfmd.`scan_id` < " . (int) $scan_id . " ORDER BY sfmd.`scan_id` DESC LIMIT 0, 1");
	...
	public function getScan(int $scan_id, bool $full = false): array {
		if ($full) {
			$result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "security_filesystem_monitor_generated` AS sfmg LEFT JOIN `" . DB_PREFIX . "security_filesystem_monitor_data` AS sfmd ON sfmd.`scan_id` = sfmg.`scan_id` WHERE sfmg.`scan_id` = " . (int) $scan_id);
	...
	public function deleteScan(int $scan_id): void {
	...
	public function getScans(array $data = []): array {
	...
	public function rename(int $scan_id, string $scan_name): int {
	...
	public function updateScansData(array $scans): void {
	...
	public function install(bool $replace = false): void {
