...
class ControllerModuleFsMonitor extends Controller {
	...
	public function index() {
		...
		if (version_compare('2.2', VERSION) >= 0) {
			$this->response->setOutput($this->load->view($this->_route . '/main.tpl', $data));
		} else {
	...
	public function viewScan() {
		...
		if (version_compare('2.2', VERSION) >= 0) {
			$this->response->setOutput($this->load->view($this->_route . '/view_scan.tpl', $data));
		} else {
	...
	public function settings() {
		...
		if (version_compare('2.2', VERSION) >= 0) {
			$this->response->setOutput($this->load->view($this->_route . '/settings.tpl', $data));
		} else {
	...
	public function viewFile() {
		...
		if ($this->user->hasPermission('modify', $this->_route)) {
			...
			if (version_compare('2.2', VERSION) >= 0) {
				$this->response->setOutput($this->load->view($this->_route . '/view_file.tpl', $data));
			} else {
	...
	public function widget() {
		...
		if ($scan) {
			...
			if (version_compare('2.2', VERSION) >= 0) {
				return $this->load->view($this->_route . '/widget.tpl', $data);
			} else {
