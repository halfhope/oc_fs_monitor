...
class ModelSecurityFSMonitor extends Model
{
    ...
    public function addScan($name, $files, $scan_size)
    {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "security_filesystem_monitor_generated` (scan_id, scan_size, user_name, name, auto, date_added) VALUES (" . (int) $scan_id . "," . (int) $scan_size . ",'" . $this->db->escape($this->user->getUserName()) . "','" . $this->db->escape($name) . "', 0, '" . date('Y-m-d H:i:s') . "');");
