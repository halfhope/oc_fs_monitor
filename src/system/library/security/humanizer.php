...
class Humanizer {
	...
	function __construct(object $registry) {
		$this->language = $registry->get('language');
	}

	public function humanDateDifference(string $date_start, string $date_end): string {
	...
	public function humanBytes(int $size): string {
		$filesizename = [
			" B",
			" KB",
			" MB",
	...
	public function humanDatePrecise(string $date, string $format = 'Y-m-d H:i:s'): string {
		$r = false;
		$a = preg_split("/[:\.\s-]+/", $date);
